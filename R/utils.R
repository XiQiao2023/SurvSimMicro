...
.as_index <- function(index, p, colnames) {
  if (is.character(index)) {
    if (length(index) == 0L || anyNA(index) || any(!nzchar(index)))
      stop("'index' must contain at least one non-missing column name.")
    if (is.null(colnames))
    ...
    if (!is.numeric(index) || length(index) == 0L || anyNA(index) ||
      ...
      stop("'index' must be a non-empty vector of finite integer positions.")
