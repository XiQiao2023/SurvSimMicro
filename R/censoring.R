...
sim_censor <- function(latent_time,
  ...
  if (any(is.na(latent_time)) || any(finite_times < 0))
    stop("'latent_time' must contain non-negative finite times or Inf.")

  if (!is.null(administrative_time)) {
    if (!is.numeric(administrative_time) || length(administrative_time) != 1L ||
        !is.finite(administrative_time) || administrative_time <= 0)
      stop("'administrative_time' must be a single positive finite number.")
  }
  ...
  if (!is.null(target_event_prop) && uses_admin) {
    ...
    if (length(finite_times) == 0L)
      stop("Cannot calibrate 'administrative_time' because all latent failure ",
           "times are Inf.")
  ...
  censor_time <- switch(
    ...
    none = {
      if (length(finite_times) == 0L)
        stop("type = 'none' needs at least one finite latent failure time.")
